...
2. <action> (e.g. scaffold UserList feature using <pattern> skill's reference impl)
...
Suggested next reference: references/<pattern>.md
```
...
After delivering the recommendation, suggest the user read `references/<pattern>.md` to scaffold the first feature, and (if migration from any existing code is in play) point to `migrator`.
