...
- Use this when UIKit screens need observable state and Combine is acceptable.
- Reject it when state is global or action-heavy; use TCA or Redux/ReSwift instead.
- First safe slice: move loading, paging, and error state into a ViewModel while keeping the existing ViewController.
...
// Features/UserDetail/UserDetailViewController.swift — see mvvm-c (same shape)
```

(See `mvvm-c` for `UserDetailViewController` and Coordinator integration — they're identical.)
...
- Unit test the ViewModel with a fake repository and assert published state in order.
- Cover repository failure and retry paths by asserting `errorMessage`, loading reset, and no stale rows.
- Keep Combine expectations scheduler-controlled where timing matters; avoid sleeping tests.
- Test navigation through emitted events (`userSelected`, route closures), not by pushing UIKit controllers.
...
| --- | --- | --- |
| Initial list load | Assert the ViewModel publishes loading and loaded states for the first fake page. | FakeUserRepository plus Combine expectation |
| Repository failure | Assert the error publisher emits a user-facing state and does not complete the state stream. | FakeUserRepository error branch |
| Pull to refresh | Assert refresh replaces items instead of appending duplicate page-one rows. | FakeUserRepository fetchPageCalls |
| Detail save | Assert save publishes the updated user and clears the saving flag. | FakeUserRepository updateCalls |

## Concurrency and cancellation

- Keep the ViewModel `@MainActor` when it owns UI-facing state.
- Cancel previous load/save tasks before starting replacement work; pagination must not let old pages append after refresh.
- UIKit bindings should receive on the main queue and be stored in controller lifecycle-owned cancellables.
