...
- Use this when build graph, ownership, and feature isolation are the main architectural problem.
- Reject it when one app target and one team can still move quickly without module boundaries.
- First safe slice: split one feature into Interface and Sources, then add a graph check before adding more modules.
...
- Test feature modules through their `Interface` contracts and keep source-only internals covered by module tests.
- Assert public feature state and outputs through the `Interface` target, not by importing another module's internals.
- Cover interface-level failure cases so module internals cannot leak as error handling contracts.
- Use fake Interface implementations for dependency seams; never import another module's `Sources` in tests.
- Add build-graph checks in CI so feature modules cannot import another feature's `Sources`.
- Keep shared test support in dedicated test-only targets, never in production interfaces.
- Keep module tests deterministic by faking cross-module interfaces instead of reaching into `Sources`.
...
| --- | --- | --- |
| Initial list load | Assert feature sources depend on Interface contracts and load fake user data. | Fake Interface dependency plus package test |
| Repository failure | Assert failure stays within the feature boundary and public Interface remains stable. | Feature test plus Interface import check |
| Module boundary | Assert sibling features import Interface targets, not each other's Sources. | SPM/Tuist graph check plus Interface dependency seam |
| Detail save | Assert save uses the injected Interface contract and updates local feature state. | Fake Interface updateCalls |

## Concurrency and cancellation

- Async work should belong to the feature or service module that owns the lifetime; do not leak task ownership across module interfaces.
- Public interfaces should express cancellation through async APIs, effect handles, or explicit lifecycle calls.
- Cross-module callbacks must return to the main actor before touching UI-facing state.
...
- Combine with any in-module pattern: see `mvvm-swiftui`, `clean-architecture`, `tca`, etc.
