...
//
//   xcodebuild build -scheme MVCExample -destination 'generic/platform=iOS'
//   xcodebuild test  -scheme MVCExample -destination 'platform=iOS Simulator,name=iPhone 17 Pro'
//
...
let package = Package(
    ...
    platforms: [.iOS(.v17)],
    products: [
        .library(name: "MVCExample", targets: ["MVCExample"])
    ],
    targets: [
        .target(
            name: "MVCExample",
        ...
        .testTarget(
            name: "MVCExampleTests",
            dependencies: ["MVCExample"],
            path: "Tests"
        )
    ]
)
