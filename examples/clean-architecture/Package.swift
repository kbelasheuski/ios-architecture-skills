...
//
//   xcodebuild build -scheme CleanArchitectureExample -destination 'generic/platform=iOS'
//   xcodebuild test  -scheme CleanArchitectureExample -destination 'platform=iOS Simulator,name=iPhone 17 Pro'
//
...
let package = Package(
    ...
    platforms: [.iOS(.v17)],
    products: [
        .library(name: "CleanArchitectureExample", targets: ["CleanArchitectureExample"])
    ],
    targets: [
        .target(
            name: "CleanArchitectureExample",
        ...
        .testTarget(
            ...
            dependencies: ["CleanArchitectureExample"],
            path: "Tests"
        )
    ]
)
